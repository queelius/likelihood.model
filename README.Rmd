...
We can retrieve the total log-likelihood and score functions with:
```{r case-1-likelihood-model-ll-s}
ll.exp <- loglik(model.exp)
s.exp <- score(model.exp)
```


The MLE is just:
```{r case-1-mle-likelihood}
exp.mle <- mle_numerical(optim(rate, fn = ll.exp, gr = s.exp,
    df = df, hessian = TRUE,
    lower = 0, upper = 5, method = "Brent", control = list(fnscale = -1)))
summary(exp.mle)
```
...
```{r case-1-likelihood-model-explicit}
...
```{r case-1-likelihood-model-explicit-left-censored, eval=F}
