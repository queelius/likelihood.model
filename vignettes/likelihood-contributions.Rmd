...
Note that `likelihood_model` objects have a default method for `mle`. Internally,
this method uses `optim` to maximize the log-likelihood, but it packs in some
additional information and returns the result as an `mle_numerical` object.
```{r mle-observed-likelihood-model, cache=TRUE}
mle.solver_observed <- mle(model.observed)
mle.observed <- mle.solver_observed(df, par0 = rates)
summary(mle.observed)
loglik(model.observed)(df, point(mle.observed))
```
...
H <- -algebraic.mle::fim(mle.observed)
H[abs(H) < 1e-3] <- 0
print(H)
...
mle.bs <- bootstrap(model.observed)

boots <- mle.bs(df, par = rates, R = 10)
```
